                  ...
                  uint16_t max_mm_distance)
{
    ...
    _min_mm_distance = _min(min_mm_distance, max_mm_distance);
    _max_mm_distance = _max(max_mm_distance, min_mm_distance);
}
...
PingSerial::data_available (void)
{
    ...
    if (available) {
        if (_distance_pending) {
            if (available < 2) {
                ...
                // Got valid data
                _max_op_duration_ms = _max(millis() - _op_started, _max_op_duration_ms);
    ...
    if (OPERATION_PENDING && (millis() > _op_started + _op_timeout_ms)) {
        ...
        _timeout_count++;
        _timeout_count = _max(_timeout_count, 1);
